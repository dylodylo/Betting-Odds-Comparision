...
def load_matches_odds(match_containers, odd_container, dates, live_matches, league_id):
    ...
    while counter+live_matches < len(match_containers):
            ...
            if database.is_match_in_db(bookie, match_id):
                if not database.compare_odds(bookie, match_id, (float(home), float(draw), float(away), float(hd), float(da), float(ha))):
                    database.update_odds(bookie, match_id, home, draw, away, hd, da, ha)

            else:
                database.insert_odds(bookie, match_id, home, draw, away, hd, da, ha)
                #zapis do bazy danych meczu (powiązanie z kursami po id)
                database.insert_match(bookie, match_id, team1, team2, date, league_id)
